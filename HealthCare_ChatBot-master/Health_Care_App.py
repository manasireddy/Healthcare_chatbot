...
class ChatApplication:
    ...
    # Load the registration file
        try:
            ...
            ws = wb.active
            # Assuming the name is in the first row and second column
            name_from_excel = ws.cell(row=1, column=5).value
    ...
    # msg entry box
        ...
        self.name_entry.insert(0, name_from_excel)  # Set initial value from Excel
    ...
    # About menu
        ...
        about.add_command(label="Develpoers",
                      command=lambda: self.msg_showinfo(f"Bot Develpoers ",
                                                        f"Project Mentor - Mr.Prashant Kumar Yadav \n\nGroup number - 01 \n\nGroup Members \n\n1. Durgesh Maurya (105501) \n2. Dimpla (185506) \n3. Dev Sharan Yadav (185532) \n"),
                      font="bold")
        about.add_command(label="About Project",
                      command=lambda: self.msg_showinfo(f"Bot V1.0.0 ",
                                                        f" \tAbout Project \n\nThe primary goal of this project is to forecast the disease so that patients can get the desired output according to their primary symptoms.\n\n GitHub Link \n\nhttps://github.com/Durgesh63/HealthCare_ChatBot.git"),
                      font="bold")

    # Quit menu
        menu.add_command(label='Quit!', font="bold", command=lambda: self.msg_msg_askcancle(f"Ok Quit ",
                                                                                         " Are You sure? "))
